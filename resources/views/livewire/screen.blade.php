...
new class extends Component {
    ...
    {
        Column::where('id', $column_id)->update([
...
<main class="flex w-full gap-8 p-4 h-dvh">
    <header
        ...
        <div
            ...
            <div class="bg-zinc-900 flex items-center gap-4">
                <h2 class="font-semibold text-xl text-white">
                    <flux:button variant="danger" wire:confirm="Are you sure you want to delete this screen?"
                        wire:click="deleteScreen">Delete {{ $screen->name }} screen</flux:button>
                </h2>
            </div>
    ...
    <div class="w-full flex justify-center h-full gap-6" wire:poll.1s>
        @foreach ($columns as $column)
            ...
            @else
                <div class="flex-1 flex flex-col gap-2 mx-auto items-center justify-center">
                    ...
                    <flux:button wire:click="deleteColumn({{ $column->id }})">Delete Column {{ $column->name }}
                    </flux:button>
                </div>
            @endif
        @endforeach
